...
//		    datatype: 'json',
			...
			rownumbers: true,
			rownumWidth:50,
			colNames:['id','','协议名称', '社保公积金','入职手续', '离职手续','变更'],
			colModel:[
			    {name:'id',index:'id',width:90,hidden:true},
			    {name:'action',index:'action',width:16},
			    {name:'协议名称',index:'协议名称',width:100},
			    {name:'社保公积金',index:'社保公积金',width:90,editable:false},
			    {name:'入职手续',index:'入职手续',width:90,editable:false},
			    {name:'离职手续',index:'离职手续',width:90,editable:false},
			    {name:'变更',index:'变更',width:140,editable:false}
			], 
			viewrecords : false,
			rowNum:10,
			rowList:[10,20,30],
		...
		$('#selectMenu').click(function(){
			...
			window.location = ('product/list?timestamp=' + new Date().getTime() );
					...
					success:function(json){
						if(json.status == "success"){
							$('#user-input').load('../common/success.jsp',
									{msg:'操作已成功，协议"<b>'+$('#username').val()+'"</b>保存成功',
									 seconds:'3000',
									 tips:''},'');
