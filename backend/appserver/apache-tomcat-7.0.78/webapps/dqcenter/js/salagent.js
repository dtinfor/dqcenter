...
//		    datatype: 'json',
			...
			rownumbers: true,
			rownumWidth:50,
			colNames:['id','','供应商名称', '供应商编号','供应商类型', '注册城市','开户行','开户单位名称','办公地址','分管业务部','上级单位','付费截止日','服务地区'],
			colModel:[
			    {name:'id',index:'id',width:90,hidden:true},
			    {name:'action',index:'action',width:32},
			    {name:'name',index:'name',width:100,editable:true},
			    {name:'code',index:'code',width:90,editable:false},
			    {name:'type',index:'type',width:90,editable:false},
			    {name:'regCity',index:'regCity',width:90,editable:false},
			    {name:'regBank',index:'regBank',width:90,editable:false},
			    {name:'regBankUnit',index:'regBankUnit',width:140,editable:false},
			    
			    {name:'办公地址',index:'办公地址',width:140,editable:false},
			    {name:'分管业务部',index:'分管业务部',width:140,editable:false},
			    {name:'上级单位',index:'上级单位',width:140,editable:false},
			    {name:'付费截止日',index:'付费截止日',width:140,editable:false},
			    {name:'服务地区',index:'服务地区',width:140,editable:false}
			], 
			viewrecords : false,
			rowNum:10,
			rowList:[10,20,30],
...
//				$('#btn_selectMenu').button('loading');
				...
				window.location = ('product/list?timestamp=' + new Date().getTime() + "&id=" + rowIds[0]);
...
//				$('#btn_selectMenu').button('loading');
		...
		//选择 Aera
		$('#selectAera').click(function(){
			
			var rowIds = getJqgridSelected("grid-table");
			
			$('#user-input').removeData("bs.modal");
			$('#user-input').modal({
				remote:'selectAera?timestamp=' + new Date().getTime() + "&id=" + rowIds[0],
				backdrop:'static'
			});
...
//			alert(isValid);
				...
				$.ajax({
					...
					success:function(json){
						if(json.status == "success"){
							$('#user-input').load('../common/success.jsp',
									{msg:'操作已成功，供应商"<b>'+$('#username').val()+'"</b>保存成功',
									 seconds:'3000',
									 tips:''},'');
	...
	commAlert:function(modalId,msg){
		$(modalId).load('../common/alert.jsp',{msg:msg,seconds:'3000',tips:''},'');
	},
	commDelete:function(modalId,msg,delIds){
		delIds = delIds.join(",");      
		$(modalId).load('../common/deleteRequest.jsp',
				{msg:msg,
				 delIds:delIds,
				 modalId:modalId,
